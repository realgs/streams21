...
START = int(input('Podaj poczatek przedziału y do wiliczenia rsi:'))
STOP = int(input('Podaj koniec przedziału y do wiliczenia rsi:'))
...
def average(data_list, n):
    part_data = data_list[len(data_list)-n:]
    avg = sum(part_data)/len(part_data)
    return avg


def count_rsi(data_list, start, stop):
    part_data = data_list[start:stop]
    rises = 0
    rises_counter = 0
    losses = 0
    losses_counter = 0
...
def add_data(currency, buy_list, sell_list, avg_buy_list, avg_sell_list, volumen_list, rsi_buy_list, rsi_sell_list):
    buy, sell, volumen = get_data(currency)
    buy_list.append(buy)
...
def make_plot(a):
    y1, y2, avg1, avg2, v, rsi1, rsi2 = add_data(CURRENCY[0], buys, sells, avg_buy, avg_sell, volume, rsi_buy_values, rsi_sell_values)
