...
def get_volume(currency_pair):
    ...
    try:
        req = requests.get(f'https://api.bitbay.net/rest/trading/candle/history/{currency_pair}/60?from={now - VOLUME_INTERVAL}&to={now}')
        if req.status_code in CODES:
            data = req.json()
            vol = 0
            for i in data['items']:
...
class App():
    ...
    def draw_app(self):
        ...
        image = Image.open(path +'\\market.png')
        image = image.resize(self.IMGE_SHAPE)
        ...
        self.load_button = tk.Button(self.left_frame, text = 'Zamknij program', command=self.quit).grid(row=1, column = 3)
        ...
        self.width_input.insert(0,str(self.MAX_POINTS))
        ...
        self.mean_input.insert(0,str(self.SAMPLE_SIZE))

        self.rsi_mean_label = tk.Label(self.right_frame, text='Średnia RSI z:').grid(row = 0, column=2)
        self.rsi_mean_input = tk.Entry(self.right_frame)
        self.rsi_mean_input.grid(row = 1, column=2)
        self.rsi_mean_input.insert(0,str(self.RSI_SAMPLE_SIZE))
        ...
        self.currency_amount = tk.Label(self.sell_values_frame, text='')
        self.currency_amount.grid(row=2, column=1)
    ...
    def update_values(self):
        ...
        self.RSI_SAMPLE_SIZES = int(self.rsi_mean_input.get())
    ...
    def generate_plot(self, currency_pairs):
        ...
        for i in range(self.n):
            ...
            self.data_lines.append(self.axs[2*i+1].plot([], [],'k-', label='RSI'))
            ...
            self.text_frames.append(self.axs[2*i].text(1.02,0.1,'', transform=self.axs[2*i].transAxes))
        ...
        plt.savefig(str(pathlib.Path(__file__).parent.absolute())+'\\market.png',bbox_inches='tight')
    ...
    def calc_avg_buy(self, id):
        if len(self.buy_history[id]) == 0:
            self.buy_mean[id] = None
        else:
            self.buy_mean[id] = sum(self.buy_history[id]) / len(self.buy_history[id])
    ...
    def print_storage(self):
        ...
        for i in range(len(self.CURR)):
            self.storage_data += 'Posiadzasz ' + str(len(self.buy_history[i])) + ' ' +str(self.CURR[i]) + '\n'
        self.currency_amount['text'] = self.storage_data
    ...
    def update_plot(self):
        ...
        for i in range(len(self.currency_pairs)):
            ...
            print(self.buy_history[i])
            print(self.buy_mean[i])
            if self.buy_mean[i]:
                self.data_lines[i*6+4][0].set_data(self.date[i], self.buy_mean[i])
            else:
                self.data_lines[i*6+4][0].set_data([],[])
            ...
            if len(self.rsi[i]) > 2:
                if self.rsi[i][-1] > self.rsi[i][-2]:
                    ...
                    self.trend = 'decrease'
                    self.trends[i] = 0
                self.text_frames[i*5].set_text(self.trend)
                if self.income[i] != 0:
                    self.text_frames[i*5+4].set_text('Zysk: '+str(round(self.income[i],1)))
                else:
                    self.text_frames[i*5+4].set_text('')
        ...
        if len(self.rsi[0]) > 2:
            ...
            for k in range(len(vol)):
                if k == max_id and self.trends[k] != 0:
                    self.text_frames[k*5 + 1].set_text('candidate')
                    fluctuation = (abs(max(self.buy[k][-Y:])-min(self.sell[k][-Y:])) / max(self.buy[k][-Y:]))*100
                    if fluctuation > X:
                        self.text_frames[k*5 + 2].set_text('volatile asset')
                    else:
                        self.text_frames[k*5 + 2].set_text(' ')

                    spread = (abs(self.buy[k][-1]-self.sell[k][-1]) / self.buy[k][-1])*100
                    if spread > S:
                        self.text_frames[k*5 + 3].set_text('liquid asset')
                    else:
                        self.text_frames[k*5 + 3].set_text(' ')
                else:
                    self.text_frames[k*5 + 1].set_text(' ')
                    self.text_frames[k*5 + 2].set_text(' ')
                    self.text_frames[k*5 + 3].set_text(' ')
