...
def checkWhatTrend(RSIArray):
    if (RSIArray[-1] < RSIArray[-2]) and (RSIArray[-2] < RSIArray[-3]):
        ...
        return "Rising trend"
    elif ((RSIArray[-1] > RSIArray[-2]) and (RSIArray[-2] < RSIArray[-3])) or (
            (RSIArray[-1] < RSIArray[-2]) and (RSIArray[-2] > RSIArray[-3])):
...
def average_from_json():
    ...
    for i in range(len(data["LTC"])):
        price_of_one = data["LTC"][i][1] * data["LTC"][i][0]
        quantity_list_ltc.append((data["LTC"][i][0]))
        pricing_list_ltc.append(price_of_one)
    if len(pricing_list_ltc) < 1:
        our_ltc_mean = None
    else:
        our_ltc_mean = (sum(pricing_list_ltc)) / (sum(quantity_list_ltc))

    for i in range(len(data["DASH"])):
        price_of_one = data["DASH"][i][1] * data["DASH"][i][0]
        quantity_list_dash.append((data["DASH"][i][0]))
        pricing_list_dash.append(price_of_one)
    if len(pricing_list_dash) < 1:
        our_dash_mean = None
    else:
        our_dash_mean = (sum(pricing_list_dash)) / (sum(quantity_list_dash))

    openfile.close()

    return our_eth_mean, our_ltc_mean, our_dash_mean


def profit_calc():
    with open('selljson.json', 'r') as openfile:
        profit_data = json.load(openfile)

        eth_profit = profit_data["ETH"]
        ltc_profit = profit_data["LTC"]
        dash_profit = profit_data["DASH"]
...
def plot():
    ...
    lines.append(ax[0][0].plot(time_list_ETH, our_eth_avg, color='orange', label='my avg', linestyle='dotted'))

    lines.append(ax[2][0].plot(time_list_ETH, RSI_list_ETH, color='brown', label='RSI ETH'))
    ax[0][0].set_title("Quotation Chart ETH PLN")
    ax[2][0].set_title("Rsi Value")
    ax[0][0].legend(loc=1)
    ...
    lines.append(ax[0][1].plot(time_list_LTC, our_ltc_avg, color='orange', label='my avg', linestyle='dotted'))
    lines.append(ax[2][1].plot(time_list_LTC, RSI_list_LTC, color='brown', label='RSI LTC'))

    ax[0][1].set_title("Quotation Chart LTC PLN")
    ax[0][1].legend(loc=1)
    ax[0][1].xaxis.set_major_formatter(DateFormatter('%H:%M:%S'))
    ax[2][1].set_title("Rsi Value")
    ...
    lines.append(ax[0][2].plot(time_list_DASH, our_dash_avg, color='orange', label='my avg', linestyle='dotted'))
    lines.append(ax[2][2].plot(time_list_DASH, RSI_list_DASH, color='brown', label='RSI LTC'))
    ax[0][2].set_title("Quotation Chart DASH PLN")
    ax[0][2].legend(loc=1)
    ax[0][2].xaxis.set_major_formatter(DateFormatter('%H:%M:%S'))
    ax[2][2].set_title("Rsi Value")
    ...
    _ = FuncAnimation(fig, func=update_plot, interval=3000)
    plt.autoscale()
    plt.show()
...
def update_plot(i):
    ...
    averages = average_from_json()
    print(averages)
    our_eth_avg.append(averages[0])

    our_ltc_avg.append(averages[1])
    our_dash_avg.append(averages[2])

    profits = profit_calc()
    eth_profit = profits[0]
    ltc_profit = profits[1]
    dash_profit = profits[2]

    print(eth_profit, ltc_profit, dash_profit)

    ax[0][0].set_title(f'ETH values, profit is: {eth_profit}')
    ax[0][1].set_title(f'LTC values, profit is: {ltc_profit}')
    ax[0][2].set_title(f'DASH values, profit is: {dash_profit}')
    ...
    lines[4][0].set_data(time_list_ETH, our_eth_avg[-1])
    lines[5][0].set_data(time_list_ETH, RSI_list_ETH)
    lines[6][0].set_data(time_list_ETH, ETHVolume)
    ...
    lines[11][0].set_data(time_list_LTC, our_ltc_avg[-1])
    lines[12][0].set_data(time_list_LTC, RSI_list_LTC)
    lines[13][0].set_data(time_list_LTC, LTCVolume)
    ...
    lines[18][0].set_data(time_list_DASH, our_dash_avg[-1])
    lines[19][0].set_data(time_list_DASH, RSI_list_DASH)
    lines[20][0].set_data(time_list_DASH, DASHVolume)
