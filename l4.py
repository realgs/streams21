...
def list_average(list_, elements_a):#blad krotkosci
    data = list_[-elements_a:]
    if elements_a > len (list_):
        print ("Error: the number of elements is smaller than list length")
        print(data)
        #exit() 
    # data = list_[-elements_a:]
...
def draw_plot(time_interval):
    ...
    while True:
        ...
        if len(buy) and len(sell) and len(buy_RSI) and len(sell_RSI) >= 2:
            ...
            axs[0].plot(x, selling_cost, label="Selling cost", color='red')
            axs[0].plot(x, purchase_cost, label="Purchase cost", color='yellow')
            axs[0].plot(x, avg_buy_value, '--', label='Average Buy Cost', color='darkred')
            axs[0].plot(x, avg_sell_value, '--', label='Average Sell Cost', color='#9B870C')

            axs[1].bar(x, volume_value, label='Volume ', color='purple')
            axs[1].set_xticks([])

            axs[2].plot(x, RSI_buy_value, label='RSI buy', color='lightgreen' )
            axs[2].plot(x, RSI_sell_value, label='RSI sell', color='darkgreen')
