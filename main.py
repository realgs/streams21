...
def show_plots(currency_pairs, sma_buy_per, sma_sell_per, rsi_buy_per, rsi_sell_per):
    ...
    time, buy, sell, volume, sma_buy, sma_sell, rsi_buy, rsi_sell = gen_arr(n), gen_arr(n), gen_arr(n), \
    ...
    for i in range(len(currency_pairs)):
        ...
        line_sma_buy, = axs[3 * (i + 1) - 3].plot([], [], '--', label=f'SMA {sma_buy_per} buy', color='blue')
        line_sma_sell, = axs[3 * (i + 1) - 3].plot([], [], '--', label=f'SMA {sma_buy_per} sell', color='orange')
