...
volume0 = []
rsi_buy_values0, rsi_sell_values0 = [], []
t = []
...
volume1 = []
rsi_buy_values1, rsi_sell_values1 = [],[]
...
volume2 = []
rsi_buy_values2, rsi_sell_values2 = [],[]
...
def liquid(buy_list, sell_list, S):
    bid = sell_list[-1]
    ask = buy_list[-1]
...
def set_title(candidate, buy_list, sell_list, X, Y, S):
    ...
    for i in range(len(CURRENCY)):
        if CURRENCY[i] == candidate:
            title_l = liquid(buy_list[i], sell_list[i], S)
            title_v = volatile(buy_list[i], X, Y)
...
def make_plot(a):
    ...
    trend_ETH = trend(rsi_buy_values1)
    trend_LTC = trend(rsi_buy_values2)
    candidate, subtitle = choose_candidate([trend_BTC, trend_ETH, trend_LTC], [v, v1, v2])
    title1, title2, title3, title4, title5, title6 = set_title(candidate, [y1, y3, y5], [y2, y4, y6], 5, 3, 5)
    plt.clf()
    plt.ion()

    plt.suptitle(subtitle, x = 0.610, y = 1,  fontsize=18)
    plt.tight_layout()
    plt.subplot(331)
    plt.title(f'{CURRENCY[0]}  {title1} \n {title4}')
    plt.plot(t, y1, label="buys", color="#00FF00")
    plt.plot(t, y2, label="sells", color="orange")
    ...
    plt.subplot(337)
    plt.title(f'RSI  - {trend_BTC}')
    plt.plot(t, rsi1, label='buy RSI', color='#800000')
    plt.plot(t, rsi2, ':', label='sell RSI', color='#008080')
    ...
    plt.subplot(332)
    plt.title(f'{CURRENCY[1]}  {title2} \n {title5}')
    plt.plot(t, y3, label="buys", color="#00FF00")
    plt.plot(t, y4, label="sells", color="orange")
    ...
    plt.subplot(338)
    plt.title(f'RSI  - {trend_ETH}')
    plt.plot(t, rsi3, label='buy RSI', color='#800000')
    plt.plot(t, rsi4, ':', label='sell RSI', color='#008080')
    ...
    plt.subplot(333)
    plt.title(f'{CURRENCY[2]}  {title3} \n {title6}')
    plt.plot(t, y5, label="buys", color="#00FF00")
    plt.plot(t, y6, label="sells", color="orange")
    ...
    plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
    plt.subplot(339)
    plt.title(f'RSI  - {trend_LTC}')
    plt.plot(t, rsi5, label='buy RSI', color='#800000')
    plt.plot(t, rsi6, ':', label='sell RSI', color='#008080')
...
def main():
    animation = FuncAnimation(plt.figure(figsize = (17, 9)), make_plot, interval=INTERVAL)
    plt.show()
